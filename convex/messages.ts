...
/// these are the backend functions
...
export const get =query({args: {
 ...
 const currentUser =await getUserByClerkId({
  ...
  const messages=await ctx.db.query("messages").withIndex("by_conversationId",q=>q.eq(
  ...
  const messageWithUsers= Promise.all(messages.map(async message =>
  {
    ...
    if(!messageSender)
    {
        throw new ConvexError(
            "Could not find sender of message"
        )
    }
     return {
        message,senderImage: messageSender.imageUrl,
